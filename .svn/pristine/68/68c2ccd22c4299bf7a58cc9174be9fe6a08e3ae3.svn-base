...
class clientPollController extends PollController {
...
//            $user_id = 0;
//            $this->user = eUser::model()->findByPK($user_id);
//        }
    }
    ...
    public function actionAjaxGetDataForMobile() {
        foreach ($_POST as $k => $v) {
            $$k = $v;
        }
        if (!isset($id)) {
    ...
    public function actionIframe() {
        $poll = ePoll::model()->current()->questionType()->find();
