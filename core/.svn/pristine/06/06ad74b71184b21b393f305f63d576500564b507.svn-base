    ...
    public function actionSendSMS($id = NULL)
    {
        ...
        if (isset($_POST['FormSendSMS'])) {
            ...
            if ($formSendSMSModel->validate()) {
                $language = $_POST['language'];//var_dump($language); exit;
                if ($language == 'arabic') {
                    $idlang = 0;
                } else {
                    $idlang = 1;
                ...
                if($formSendSMSModel->user_id || $formSendSMSModel->phone) {
                    $response = ContactUtility::sendSMS($formSendSMSModel->user_id, $formSendSMSModel->phone, $formSendSMSModel->message, $idlang);
        ...
        $this->render('sendSMS', array('formSendSMSModel' => $formSendSMSModel, 'user_id' => $id, 'username' => $username, 'language' => $language));
    }
}
