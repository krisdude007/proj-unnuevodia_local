...
jQuery.fn.daterangepicker = function(settings){
	...
	var options = jQuery.extend({
		...
		earliestDate: Date.parse('2011-09-25'), //earliest date allowed 
		latestDate: Date.parse('0years'), //latest date allowed 
	...
	var datepickerOptions = {
		onSelect: function() { 
                        ...
                        if(rangeInput.length == 2){
                            ...
                            var da = new Date(rangeA);
                            var db = new Date(rangeB);
                            document.getElementById("startDateShown").innerHTML = "From: " + months[da.getUTCMonth()] + ' ' + da.getUTCDate() + ', ' + da.getUTCFullYear();
                            document.getElementById("endDateShown").innerHTML = "To: " + months[db.getUTCMonth()] + ' ' + db.getUTCDate() + ', ' + db.getUTCFullYear();
                            document.getElementById("startDateHidden").innerHTML = rangeA;
                            document.getElementById("endDateHidden").innerHTML = rangeB;
	...
	function showRP(){
		if(rp.data('state') == 'closed'){ 
			...
			options.onOpen();
                        ...
                        // start with date range open
                        $('.ui-daterangepicker-dateRange').trigger('click');
		}
	}
	function hideRP(){
