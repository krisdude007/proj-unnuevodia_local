...
markerMgr.init = function() {
    var options = {
        //'zoom': 4,
        //'center': new google.maps.LatLng(33.2231910, 43.6792910),//arabic area
...
markerMgr.showMarkers = function() {
    ...
    //panel.innerHTML = '';
    maxI = markerMgr.records.length - 1;
    for (var i = 0; i <= maxI; i++) {
         ...
         */
        //add country geocode for country only address.
        if (markerMgr.records[i].country && markerMgr.records[i].latitude === '' && markerMgr.records[i].longitude === '') {
            var geocoder = new google.maps.Geocoder();
            geocoder.geocode({'address': markerMgr.records[i].country}, function(no, maxNo) {
                return (function(results, status) {
                    if (status === google.maps.GeocoderStatus.OK) {
                        markerMgr.records[no].latitude = results[0].geometry.location.lat();
                        markerMgr.records[no].longitude = results[0].geometry.location.lng();
                        markerMgr.addMarkers(no);
                        if (no === maxNo) {
...
markerMgr.addMarkers = function(i) {
    var latLng = new google.maps.LatLng(markerMgr.records[i].latitude, markerMgr.records[i].longitude);
...
markerMgr.time = function() {
    ...
    //var start = new Date();
    //check if all markers loaded
    if (markerMgr.markers.length < markerMgr.records.length) {
        window.setTimeout(markerMgr.time, 10);
        return;
    }
