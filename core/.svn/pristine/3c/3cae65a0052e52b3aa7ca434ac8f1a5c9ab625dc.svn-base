...
class VideoUtility {
    ...
    private static function fixOrientation($fileOutput) {
        ...
        if ($attemptRotation) {
            ...
            switch ($orientation) {
                case 90:
                    exec(Yii::app()->params['paths']['ffmpeg'] . ' -y -i ' . $fileOutput . ' -vf "transpose=1" ' . $tmpFile);
                    copy($tmpFile, $fileOutput);
                    unlink($tmpFile);
                    break;
                case 180:
                    exec(Yii::app()->params['paths']['ffmpeg'] . ' -y -i ' . $fileOutput . ' -vf "transpose=2,transpose=2" ' . $tmpFile);
                    copy($tmpFile, $fileOutput);
                    unlink($tmpFile);
                    break;
                case 270:
                    exec(Yii::app()->params['paths']['ffmpeg'] . ' -y -i ' . $fileOutput . ' -vf "transpose=2" ' . $tmpFile);
                    copy($tmpFile, $fileOutput);
                    unlink($tmpFile);
                    break;
                default:
