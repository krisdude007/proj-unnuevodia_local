...
class eGameChoiceAnswer extends GameChoiceAnswer {
    ...
    //public $responses;
    public $responses_paid;
    public $responses_free;
    ...
    public function rules() {
        ...
        return array(
            ...
            array('user_id, label, answer', 'required'),
            array('game_choice_id, user_id, point_value, is_correct', 'numerical', 'integerOnly' => true),
            array('answer, img_url', 'length', 'max' => 256),
            array('label', 'length', 'max' => 20),
            array('is_correct, point_value', 'default', 'value' => 0),
            array('created_on, updated_on', 'default', 'value' => date("Y-m-d H:i:s"), 'setOnEmpty' => false, 'on' => 'insert'),
            array('updated_on', 'default', 'value' => date("Y-m-d H:i:s"), 'setOnEmpty' => false, 'on' => 'update'),
            array('id, game_choice_id, user_id, label, answer, img_url, point_value, is_correct, created_on, updated_on', 'safe', 'on' => 'search'),
        );
    }
    ...
    public function afterFind() {
        ...
        $this->responses_paid = eGameChoiceResponse::model()->isPaid()->count("game_choice_id={$this->game_choice_id} AND game_choice_answer_id={$this->id}");
        $this->responses_free = eGameChoiceResponse::model()->isFree()->count("game_choice_id={$this->game_choice_id} AND game_choice_answer_id={$this->id}");
