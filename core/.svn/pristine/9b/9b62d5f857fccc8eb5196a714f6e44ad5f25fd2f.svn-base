...
class ePrize extends Prize {
    ...
    public static function getRandomActivePrizes() {

        $max = self::model()->count(array(
            'condition' => 'enabled = 1 and quantity >=1',
        ));
        ...
        $criteria->limit = 3;
        $prizes = self::model()->isActive()->hasQuantity()->findAll($criteria);
