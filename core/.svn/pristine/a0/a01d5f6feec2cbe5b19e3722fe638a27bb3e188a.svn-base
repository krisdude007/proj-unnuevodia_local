...
class eQuestion extends Question {
    ...
    public function relations() {
        ...
        if (Yii::app()->params['video']['useExtendedFilters']) {
            $videoExtendedRelation = array(
                'videoAcceptedTvTally' => array_merge(array(self::STAT, 'eVideo', 'question_id', 'select' => 'COUNT(id)', 'group' => 'question_id'), Utility::getAcceptedTVScope(Yii::app()->params['video']['extendedFilterLabels'])),
            );
        } else {
            $videoExtendedRelation = array(
        ...
        if (Yii::app()->params['ticker']['useExtendedFilters']) {
            $tickerExtendedRelation = array(
                'tickerAcceptedTvTally' => array_merge(array(self::STAT, 'eTicker', 'question_id', 'select' => 'COUNT(id)', 'group' => 'question_id'), Utility::getAcceptedTVScope(Yii::app()->params['ticker']['extendedFilterLabels'])),
            );
        } else {
            $tickerExtendedRelation = array(
    ...
    public function search($type) {
        ...
        if (Yii::app()->params[strtolower($type)]['useExtendedFilters']) {
            ${$type . 'ExtendedCondition'} = current(Utility::getAcceptedTVScope(Yii::app()->params[strtolower($type)]['useExtendedFilters'], 'tn'));
        } else {
