    ...
    public function actionUpload() {
        ...
        foreach ($_POST['FormBannerUpload'] as $bannerId => $file) {
            ...
            if ($type !== 3) {//file not png
                $model->addError($bannerId, 'Type must be .' . Yii::app()->params['custom_params']['banner_extension']);
                continue;
            }
            if ($width . 'x' . $height !== Yii::app()->params['banner_dimension']) {//file not correct width x height
                $model->addError($bannerId, 'Size must be ' . Yii::app()->params['banner_dimension']);
                continue;
            }
            $fileName = Yii::app()->params['paths']['image'] . '/' . Yii::app()->params['custom_params']['banner_prefix'] . '_' . $bannerId . "." . Yii::app()->params['custom_params']['banner_extension'];
