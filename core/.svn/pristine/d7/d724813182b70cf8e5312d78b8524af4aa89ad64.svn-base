...
//            'socialToTvCount' => eTicker::model()->filterByTickerDate($filterDate)->social()->with('tickerDestination')->filterByDestination($tvId)->count(),
    ...
    public function actionGamePlayesReport() {
        ...
        header("Content-Type: application/octet-stream");
        header("Content-Type: application/download");
        header("Content-Disposition: attachment;filename=GameUniqueUser_Report.csv");
        header("Content-Transfer-Encoding: binary");
        
        $responces = eGameChoiceResponse::model()->with('user')->FindAllByAttributes(Array('game_choice_id' => $game_id));
        
        $headerArray = array('User ID', 'Username', 'Game ID', 'Game Answer', 'Winner', 'Source', 'Date/Time');
        ...
        {
            $answers = eGameChoiceAnswer::model()->findByPK($r->game_choice_answer_id);
            ...
            $row = array($r->user_id, $r->user->username, $r->game_choice_id, $answer, $r->is_winner, $r->source, $r->created_on);
            fputcsv($df, $row);
        }
