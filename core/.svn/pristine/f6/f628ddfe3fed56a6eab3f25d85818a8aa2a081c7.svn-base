...
class UserController extends Controller {
    ...
    public function actionIndex($order = 'recent') {
        ...
        if (Yii::app()->params['pagination']['enablePagination'] == true) {
            $total = eVideo::model()->accepted()->count($criteria);
        ...
        $videos = eVideo::model()->accepted()->findAll($criteria);
        $question = '';
        ...
        $this->render('index', array(
            ...
            'question' => $question,
            'pages' => $pages,
            'page_size' => Yii::app()->params['pagination']['listPerPage'],
            'items_count' => $total,
        ));
    }
                    ...
                    if (!empty($userRecord)) {
                        ...
                        if ($reset->save()) {
                            ...
                            if ($result) {
                                ...
                                Yii::app()->user->setFlash('error', Yii::app()->params['flashMessage']['resetPasswordError']);
                            }
                        } else {
                            Yii::app()->user->setFlash('error', Yii::app()->params['flashMessage']['resetPasswordError']);
                        }
    ...
    public function actionAjaxFacebookConnectNoSigReq() {
        if (!$userFacebook = eUserFacebook::model()->findByAttributes(array('user_id' => Yii::app()->user->getId()))) {
            $userFacebook = new eUserFacebook;
        }
        FacebookUtility::connectNoSigReq($userFacebook);
    }
    ...
    public  function actionAjaxTwitterConnection() {
        if (Yii::app()->user->isGuest) {
            echo(json_encode(array('status' => 0)));return;
        }
        $user_id = Yii::app()->user->getId();
        $user = eUser::model()->with('userTwitters')->findByPK($user_id);
        if (isset($user->userTwitters[0]->oauth_token) && $user->userTwitters[0]->oauth_token) {
            echo(json_encode(array('status' => 1)));return;
            //return 0 if token not valid.
        }
        echo(json_encode(array('status' => 0)));return;
    }

    public  function actionAjaxFacebookConnection() {
        if (Yii::app()->user->isGuest) {
            echo(json_encode(array('status' => 0)));return;
        }
        $user_id = Yii::app()->user->getId();
