...
class XMLController extends Controller {
    ...
    public function actionVoting($id = 0) {
        if($id != 0) {
            $model = ePoll::model()->with('pollAnswers', 'pollAnswers:tally', 'tally')->findByPK($id); // poll by id
        }
        else {
            $model = ePoll::model()->current()->with('pollAnswers', 'pollAnswers:tally', 'tally')->findAll(); // current poll
            $model = $model[0]; //take 1st one
        }
    ...
    public function actionGameChoice($id = 0) {
        $model = $game = eGameChoice::model()->with('gameChoiceAnswers')->findByPK($id);
        header("Content-Type: application/xml");
        $doc = new DOMDocument("1.0", "ISO-8859-1");
        $data = $doc->createElement('data');
        if (!is_null($model)) {
            $game = $doc->createElement('game');
            $game->appendChild($doc->createElement('id', $model->id));
            $game->appendChild($doc->createElement('question', $model->question));
            $game->appendChild($doc->createElement('date', $model->created_on));
            $data->appendChild($game);
            foreach ($model['gameChoiceAnswers'] as $t) {
                $answer = $doc->createElement('answer');
                $answer->appendChild($doc->createElement('id', $t->id));
                $answer->appendChild($doc->createElement('answer', $t->answer));
                $answer->appendChild($doc->createElement('points', $t->point_value));
                $answer->appendChild($doc->createElement('date', $t->created_on));
                $answer->appendChild($doc->createElement('responses', $t->responses_paid));
    ...
    public function actionQuestionTicker($id = 0) {
        ...
        if (count($questionModel) != 0) {
            ...
            foreach ($combinedTickers as $t) {
                ...
                $ticker->appendChild($doc->createElement('username', isset($user->username) ? $user->username : TickerUtility::getUsername($t)));
                ...
                $image = $doc->createElement('image');
                $imageurl = $doc->createAttribute("url");
                $image->appendChild($imageurl);
                $url = $doc->createTextNode(TickerUtility::getAvatar($t));
                $imageurl->appendChild($url);
                //$image->appendChild($doc->createElement('src', TickerUtility::getAvatar($t)));
                $ticker->appendChild($image);
    ...
    public function actionQuestionTickerRSS($id = 0) {
        ...
        if (count($questionModel) != 0) {
            ...
            foreach ($combinedTickers as $t) {
                ...
                $item = $doc->createElement('item');
                if (Yii::app()->params['custom_params']['rss_divide_title_desc'] === true) {
                ...
                if (Yii::app()->params['custom_params']['rss_powered_by_youtootech']) {
                    if ($counter % 10 == 0) {
                        $item = $doc->createElement('item');
