...
var importImageHandler = function(){
    
    var imageImportTrigger = $("#imageImportOverlay").overlay({
        mask: '#000',
        effect: 'default',
        top: 25,
        closeOnClick: true,
    ...
    $('#fab-import-button').on('click',function(e){
        e.preventDefault();
        $("#imageImportOverlay").overlay().load();
    });
    ...
    $('.fab-import-source-button').on('click',function(e){
        ...
        hashtag = hashtag.val();
        $("#imageImportOverlay").overlay().close();
        ...
        var request = $.ajax({
            url: '/adminImage/ajaxImageImport',
            type: 'POST',
            data:({
...
var overlayHandlers = function() {
    ...
    $("a[rel], button[rel]").overlay({
        ...
        onBeforeLoad: function() {
            ...
            thumbnailId = this.getTrigger().attr("id");
            thumbnailId = thumbnailId.replace("img_","img");
            ...
            //console.log(this.getTrigger().find("img").attr('href'));
        }
    });
...
var imageStatusHandlers = function() {
    ...
    switch(optionVal) {
            ...
            case 'statustv' :
                    list = { "image": "Images"};
                    $('#FormFilterImage_type option').remove();
                    $.each(list, function(key, value) {
                         $('#FormFilterImage_type')
    ...
    $("#fab-select-accept").change(function() {
        ...
        switch(optionVal) {
            ...
            break;
            default :
                    list = {"all":"All", "avatar":"Avatar", "image": "Images"};
                    $('#FormFilterImage_type option').remove();
                    $.each(list, function(key, value) {
                         $('#FormFilterImage_type')
                             .append($("<option></option>")
