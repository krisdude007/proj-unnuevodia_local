...
function updateVideoPreroll(preRoll, videoId){      
    var request = $.ajax({
        url: '/adminVideo/ajaxVideoPreRoll',
        type: 'POST',
        data:({
            'preRoll': preRoll,
            'videoId': videoId,
        ...
        success: function(data){
            if(preRoll === '1')
            {
                $('#fab-modal-add-preroll-button').hide();
                $('#fab-modal-remove-preroll-button').show();
            }
            else {
                $('#fab-modal-remove-preroll-button').hide();
                $('#fab-modal-add-preroll-button').show();
            }
        },
        error: function(data){
            alert('Unable to update Pre Roll.');
            return false;
        }
    });   
}
